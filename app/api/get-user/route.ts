...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      response = NextResponse.json({ user: null, error: `Backend error: ${res.statusText}` });
      response.cookies.set("__test", "", { path: "/", maxAge: 0 });
      return response;
    }
    ...
    if (!data || !data.user?.id) {
      ...
      response = NextResponse.json({ user: null, error: "No valid user data" });
      response.cookies.set("__test", "", { path: "/", maxAge: 0 });
      return response;
    }

    // Prepare the response with user data
    response = NextResponse.json({ user: data.user });
    ...
    response = NextResponse.json({ user: null, error: "Internal server error" });
    response.cookies.set("__test", "", { path: "/", maxAge: 0 });
    return response;
  }
}
