...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (loginSuccess === 'success') {
      console.log('Login callback detected, fetching user...')
      fetchUser()
    }
  }, [fetchUser])
  ...
  const isBookmarkedByUser = useCallback(
    (projectId: string) => {
  ...
  const hasUserClaimedProject = useCallback(
    (projectId: string) => {
      if (!user) return false
      const claimant = getProjectClaimant(projectId)
        ...
        <div className={styles['background-image-container']}>
          <div className={styles['main-text']}>
            <div className={styles.heading}>
              <h1>
                Unlock Your Final Year<br />Project Potential
              </h1>
            </div>
            <div className={styles.description}>
              <p>
                Get personalized project topic recommendations tailored to your interests and skills.
              </p>
            </div>
            <div className={styles['explore-btn']}>
              {loading ? (
                ...
                <button
                  ...
                  onClick={() => {
                    window.location.href = `${googleLoginUrl}?t=${new Date().getTime()}`
                  }}
                >
                  Sign in with Google
                </button>
        ...
        <div className={styles['tabs-container']}>
            ...
            <div className={styles['left-tabs']}>
              {filteredProjects.length > 0 ? (
                filteredProjects.map((project) => {
                  ...
                  const isTrending = !project.isUserGenerated
                      ...
                      <div className={styles['tab-header']}>
                        <div className={styles['tab-icon']}>
                          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            />
                            <path
                              d="M6.5 2H20V22H6.5C5.11929 22 4 20.8807 4 19.5V2Z"
                              stroke="currentColor"
                              strokeWidth="2"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                            <path
                              d="M6.5 2C5.11929 2 4 3.11929 4 4.5V19.5C4 20.8807 5.11929 22 6.5 22"
                              stroke="currentColor"
                              strokeWidth="2"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                        ...
                        <div className={styles['project-tags']}>
                          ...
                          {project.tags?.map((tag) => (
                            ...
                            >
                              {tag === 'bookmarked' ? '📌 Bookmarked' : tag === 'taken' ? '✅ Taken' : tag}
                            </span>
                          ))}
                          ...
                          {user && (
                            <button
                              className={`${styles['bookmark-btn']} ${
                                isBookmarkedByUser(project.id) ? styles.bookmarked : ''
                              }`}
                              onClick={(e) => {
                                e.stopPropagation()
                ...
                <div className={styles['no-results']}>
                  ...
                  <button className={styles['explore-button']} onClick={() => setSearchTerm('')}>
                    Clear search
                  </button>
                </div>
              )}
          ...
          <div className={styles['right-tabs']}>
            <div className={styles['side-tab']}>
              ...
              {user && !isAsking && (
                <button className={styles['ask-button']} onClick={() => setIsAsking(true)}>
                  Ask Question
                </button>
              )}
                  ...
                  topQuestions.map(
                    (qa) =>
                      qa && (
                        <div
                          key={qa.id}
                          className={`${styles['qa-item']} ${qa.isEmergency ? styles.emergency : ''}`}
                        >
                          <div className={styles['qa-question']}>
                            <div className={styles['qa-meta']}>
                              <div className={styles['author-info']}>
                                ...
                                <div className={styles['author-details']}>
                                  <span className={styles['qa-author']}>
                                    {qa.author.name || 'Unknown User'}
                                  </span>
                                  {qa.author.reputation !== undefined && (
                                    <span className={styles['author-reputation']}>
                                      {qa.author.reputation} points
                                    </span>
                                  )}
                                </div>
                              </div>
                            ...
                            {editingQuestion === qa.id ? (
                              <div className={styles['edit-container']}>
                                ...
                                <div className={styles['edit-actions']}>
                                  ...
                                  <button className={styles['cancel-edit-button']} onClick={handleCancelEdit}>
                                    Cancel
                                  </button>
                                </div>
                              </div>
                            ...
                            <div className={styles['qa-stats']}>
                              <button className={styles['upvote-button']} onClick={() => handleUpvote(qa.id)}>
                                ▲ {qa.upvotes || 0}
                              </button>
                              <button
                                    ...
                                    <div className={styles['author-info']}>
                                      ...
                                      <div className={styles['author-details']}>
                                        ...
                                        {answer.author.reputation !== undefined && (
                                          <span className={styles['author-reputation']}>
                                            {answer.author.reputation} points
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                    <div className={styles['answer-actions']}>
                                      <span className={styles['answer-time']}>{formatDate(answer.timestamp)}</span>
                                      <button
      ...
      <footer className={styles.footer}>
        <div className={styles['footer-content']}>
          <div className={styles['footer-links']}>
            <Link href="/support" className={styles['footer-link']}>
              Support
            </Link>
